...
#pragma config(Sensor, in7,    ClawRight_Pot,  sensorPotentiometer)
...
#pragma config(Sensor, dgtl9,  BumpSwitchBack, sensorDigitalIn)
...
#pragma config(Motor,  port2,           clawRight,     tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port3,           leftOutside,   tmotorVex393_MC29, openLoop)
...
#pragma config(Motor,  port10,          rightInside,   tmotorVex393HighSpeed_HBridge, openLoop, reversed)
...
#define Lift_Pos2 600
#define Lift_Pos3 1525
#define Lift_PosRelease 1400
#define Lift_EncoderZero 0
#define Lift_HardStop 1600 -+++
volatile bool Break;
...
#define ClawLeft_Open 2148 //(150)
#define ClawLeft_Closed 3302 //(2310)
#define ClawLeft_ClosedCube 3099
#define ClawLeft_Mid 2712

#define ClawRight_Open 1637  //(150)
#define ClawRight_Closed 580 //(2310)
#define ClawRight_ClosedCube 661
#define ClawRight_Mid 1050
...
void ProgrammingSkills(){
	...
	SetLiftPosition(Lift_Pos2);
	wait1Msec(200);
	SetDriveControl(Line, -33, 2);
	wait1Msec(500);
	////90/100 score in DNatureECA BMOR Provincials
	////2 Callbacks in 's Fury FLL Ontario Provincials (No Award)
	...
	wait1Msec(200);
	// Middle CUBE
	SetDriveControl(Line, -3, 1.5);
	wait1Msec(400);
	SetDriveControl(Line, 23, 1);
	wait1Msec(400);
	ClawPos = Open;
	setClawPosition(ClawRight_Mid, ClawLeft_Mid);
	wait1Msec(400);
	...
	//wait1Msec(750);
	//ClawPos = Open;
	//setClawPosition(ClawRight_Open - 100, ClawLeft_Open + 100);
	//wait1Msec(500);
	//liftPositionOne();
	//wait1Msec(400);
	////FENCE BOTTOM MIDDLE STARS
	ClawPos = Closed;
	setClawPosition(ClawRight_ClosedCube, ClawLeft_ClosedCube);
	wait1Msec(400);
	...
	wait1Msec(400);
	ClawPos = Open;
	setClawPosition(ClawRight_Mid,ClawLeft_Open);
	wait1Msec(400);
	SetDriveControl(Line, 30, 1);
	wait1Msec(400);
	ClawPos = Closed;
...
task usercontrol(){
	startTask(liftcontrol); startTask(clawLeftcontrol); startTask(clawRightcontrol); startTask(sound); startTask(BumpSwitchHoldingPower);
...
task clawLeftcontrol(){
	int Claw_Current; float Claw_kP = .45;
	while(true){
		if(ClawLeftActive){
			//Update Values
...
task clawRightcontrol(){
	////CHANGE kP BACK TO 0.25
	int ClawRight_Current; float Claw_kP = .35;
	while(true){
		if(ClawLeftActive){
			//Update Values
